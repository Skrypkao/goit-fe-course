...
class Hamburger {
  ...
  removeTopping(topping) {
     this._toppings = this._toppings.filter(idx => idx !== topping);
    }
  ...
  calculatePrice() {
    ...
    const summTopping = priceTopping.reduce((acc, value) => acc + value, 0);
    const totalPrice = summTopping + stuffingPrice + sizePrice;
    return totalPrice;
   }
  /**
   * Узнать калорийность
  ...
  calculateCalories() {
    ...
    const summToppingCal = toppingCal.reduce((acc, value) => acc + value, 0);
    const totalCal = sizeCal + stuffingCal + summToppingCal;
    return totalCal;
  }
}
