...
const getAllNames = arr => {
  const allNames = arr.map(user => user.name);
  return allNames;
};
...
const getUsersByEyeColor = (arr, color) => {
  const eyes = arr.filter(user => user.eyeColor === color);
  return eyes;
};
...
const getUsersByGender = (arr, gender) => {
  const genderFilter = arr.filter(user => user.gender === gender);
  return genderFilter;
};
...
const getInactiveUsers = arr => {
  const activeFilter = arr.filter(user => !user.isActive);
  return activeFilter;
};
...
const getUsersWithAge = (arr, min, max) => {
  const ageFiltered = arr.filter(user => user.age >= min && user.age <= max);
  return ageFiltered;
};
...
const getTotalBalance = arr => {
  const balanceTotal = arr.reduce((acc, user) => acc + user['balance'], 0);
  return balanceTotal;
};
...
const getUsersByFriend = (arr, name) => {
  const byFriend = arr.filter(user => user.friends.includes(name));
  return byFriend;
};
...
const getAllSkills = arr => {
  const skills = arr.reduce(
    (acc, sk) => sk.skills ? acc.concat(sk.skills) : acc,
    []
  );
  const unique = skills.filter((v, i, a) => a.indexOf(v) === i); 
...
const getUserNamesSortedByFriendsCount = arr => {
  const namesByFriends = (a,b) => a.friends.length - b.friends.length;
  const sortByFriends = arr.sort(namesByFriends);
  const allNames = sortByFriends.map(user => user.name);
  return allNames;
};
